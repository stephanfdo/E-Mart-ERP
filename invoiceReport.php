...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "SELECT `invoice`.`invoice_no`, `invoice`.`customer`, `invoice`.`date`, `customer`.`district`, `invoice`.`item_count`, `invoice`.`amount`
    FROM `invoice` 
        LEFT JOIN `customer` ON `invoice`.`customer` = `customer`.`id` WHERE `invoice`.`date` BETWEEN '$start_date' AND '$end_date' ";
...
<body id="page-top">
    ...
    ?>
                    <h3 class="text-dark mb-4">Invoice Report</h3>
                    <div class="row mb-3">
    ...
    if (isset($result) && $result->num_rows > 0) {
        ...
        echo "<tr><th scope='col'>Invoice Number</th><th scope='col'>Date</th><th scope='col'>Customer</th><th scope='col'>Customer District</th><th scope='col'>Item Count</th><th scope='col'>Invoice Amount</th></tr>";
